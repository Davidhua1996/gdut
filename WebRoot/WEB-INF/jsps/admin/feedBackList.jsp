...
<body>
    ...
    <div id="content">
        ...
        <div style="float:left;margin-left:50px;padding-bottom:0px; position: relative; left: 10px;top: 13px;">
       			 用户类型
                <select style="width:85px;margin-bottom:0px" id="userType">
...
//getData方法
	function getData(){
		...
		if($("#userType").val()=="student"){
			url = 'feedBack_student.action?aa=Math.random()';
		}else if($("#userType").val()=="vendor"){
			url = 'feedBack_vendor.action?aa=Math.random()';
		}else{
			return false;
		}
		mask('正在加载中,请稍后...');
	  ...
	  type:'GET',
	     ...
	     dateType:'json',
	      ...
	      $.each(pager.recordList,function(i,el){
	       ...
	       $("<td>"+el.dateTime+"</td>").appendTo($tr);
	       $("<td>"+el.content+"</td>").appendTo($tr);
	       $("<td class='center'>"
              + "<a href='#myModal' role='button' class='btn' onclick='dele(this)' data-toggle='modal'>删除</a>" 
              +"</td>").appendTo($tr);
 ...
 //删除按钮
        function dele(e){
            ...
            $("#btn-true").bind('click',function(){
            	...
            	$.ajax({
            		url:'feedBack_delete.action?aa=Math.random()',
            		type:'GET',
            		dataType:'json',
            		data:{id:id},
            		success:function(data){
